	...
	<el-row class="container">
		...
		<el-col :span="24" class="topbar-wrap">
			...
			<div class="topbar-logos">
				<a href="/" style="color: #fff;"><span v-show="!$store.status.collapsed">车车综合管理</span></a>
			</div>
			<div class="topbar-title">
				<!-- 注意：这里就是topNavState作用之处，根据当前路由所在根路由的type值判断显示不同顶部导航菜单 -->
				<el-row v-show="$store.status.topNavState==='home'">
					<el-col :span="24">
				...
				<el-row v-show="$store.status.topNavState==='enterprise'">
					<el-col :span="24">
						<el-menu :default-active="defaultActiveIndex" class="el-menu-demo" mode="horizontal" @select="handleSelect" :router="true">
            ...
            <i class="iconfont icon-user"></i> {{nickname}}   <i class="el-icon-caret-bottom"></i>
					</span>
	...
	export default {
		...
		methods: {
			...
			fetchNavData() { // 初始化菜单激活项
				...
				this.$store.status.topNavState = nav_type;
				this.$store.status.leftNavState = nav_name;
				if(nav_type == "home") {
